...
await member.roles.add(settings.roller.kayıtsızrol)
await member.roles.add(settings.roller.kayıtsızrol)
message.channel.send(new MessageEmbed().setDescription(`${member} Adlı kullanıcı ${message.author} tarafından kayıtsıza atıldı!`).setFooter("shi")).then(msg => msg.delete({timeout: 4000}))
